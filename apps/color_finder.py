...
class ColorFinder(cozmo.annotate.Annotator):
    ...
    def drive_toward_color_blob(self):
        ...
        if self.should_start_new_action(self.drive_action):
            self.drive_action = self.robot.drive_straight(distance_mm(500), speed_mmps(300), should_play_anim=False,
                                                          in_parallel=True)
    ...
    async def run(self):
        ...
        if not self.cubes_connected():
            print(
                'Cubes did not connect successfully - check that they are nearby. You may need to replace the batteries.')
            return
        self.turn_on_cubes()
...
class BlobDetector():
    ...
    def filter_blobs_dict_by_size(self, n):
        ...
        '''
        self.blobs_dict = dict(
            (blob, list_of_points) for blob, list_of_points in self.blobs_dict.items() if len(list_of_points) >= n)
    ...
    def get_largest_blob_key(self):
        ...
        if len(values) > 0:
            longest_points_list = functools.reduce(lambda largest, current: largest if (largest > current) else current,
                                                   values)
